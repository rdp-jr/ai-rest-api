...
async def get_synthetic_dataset(project_id: str, model_id: str, synthetic_dataset_id: str, db = Depends(get_db)):
...
async def create_synthetic_dataset(project_id: str, model_id: str, db = Depends(get_db)):
...
async def update_synthetic_dataset(project_id: str, model_id: str, synthetic_dataset_id: str, req: UpdateDataset, db = Depends(get_db)):
    if not get_project_service(db, project_id):
        raise HTTPException(status_code=404, detail="Project not found")
...
async def delete_synthetic_dataset(project_id: str, model_id: str, synthetic_dataset_id: str, db = Depends(get_db)):
    if not get_project_service(db, project_id):
        raise HTTPException(status_code=404, detail="Project not found")
